...
# Copyright (C) 2012/2013 PLAIN Project
#
...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from PLAIN Github (http://github.com/PLAIN)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/PLAIN/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/plain_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'PLAIN/%s already exists' % (repo_name)
            else:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "plain", "name": repo_name, "revision": "kitkat" })
    ...
    f = open('.repo/local_manifests/plain_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for PLAIN/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: PLAIN/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "plain", "name": "PLAIN/%s" % repo_name, "revision": "kitkat" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/plain.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':'kitkat'}])
...
print "Repository for %s not found in the PLAIN Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/plain_manifest.xml" % device
